...
###############################################
#Only meant for OSTYPE=linux-gnu at the moment
...
if [ ! -z "$apt" ]; then
	...
	yum install lsb-release &> /dev/null
else
...
if [ "$os" == "CentOS" ] || [ "$os" == "Red Hat" ]; then
	...
	while true; do
		read -p "Would you like to install this update?" yn
		case $yn in
			[Yy]* ) yum update
				break;;
			[Nn]* ) break;;
...
#Asks the user if they would like to update the programs or not
	...
	while true; do
		if grep -Fxq "alias net-pf-10 off" /etc/modprobe.conf; then
			echo "IPv6 is already disabled."
		else
			read -p "Would you like to disable IPv6?" yn	
			case $yn in
				[Yy]* ) sed -i "alias net-pf-10 off" /etc/modprode.conf
					break;;
				[Nn]* ) break;;
				* ) echo "Please answer yes or no.";;
			esac
	...
	while true; do
		read -p "Would you like to install this update?" yn
		case $yn in
			[Yy]* ) apt-get update; apt-get upgrade
				break;;
			[Nn]* ) break;;
			* ) echo "Please answer yes or no.";;
		esac
